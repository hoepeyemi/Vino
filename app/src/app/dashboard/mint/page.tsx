...
import { CalendarIcon, ArrowRight, ArrowLeft, Loader2, ExternalLink, Check, AlertCircle, ShieldCheck, ShieldAlert, Coins } from "lucide-react"
import { useCleanverseCVI } from "@/hooks/use-cleanverse-cvi"

type AToken = { atoken: string; symbol: string; contractAddress?: string }
...
function MintInvoiceContent() {
  ...
  useEffect(() => {
    void fetch('/api/cleanverse/atoken-list', {
      ...
      .then(r => r.json())
      .then((data: { tokens?: AToken[] }) => { setCvaTokens(data.tokens ?? []) })
      .catch(() => {})
    ...
    <div className="min-h-screen bg-[#0a0a0a] bg-grid noise-overlay scan-line pb-8">
      ...
      <main className="max-w-3xl mx-auto px-6 py-8">
        ...
        {cviState.status === "onboarding" && (
          <div className="mb-6 p-4 rounded-lg border border-[#10b981]/30 bg-[#10b981]/5 flex items-center gap-3">
            <Loader2 className="w-4 h-4 text-[#10b981] animate-spin shrink-0" />
            <div>
              <p className="text-[12px] font-semibold text-[#10b981] uppercase tracking-wider mb-0.5">
                Processing KYB Verification
              </p>
              <p className="text-[11px] text-[#666666]">Generating A-Pass and approving on-chain…</p>
            </div>
          </div>
        )}
        ...
        {cviState.status === "verified" && (
          <div className="mb-6 p-3 rounded-lg border border-[#10b981]/30 bg-[#10b981]/5 flex items-center gap-2">
            <ShieldCheck className="w-3.5 h-3.5 text-[#10b981]" />
            <p className="text-[11px] text-[#10b981]">
              Cleanverse KYB verified · Tier {cviState.tier}
            </p>
          </div>
        )}
        ...
        {step === 2 && (
          <div className="terminal-card p-6">
            ...
            <div className="flex justify-between gap-3">
              ...
              <Button onClick={handleMint} disabled={!isConnected || isMinting || cviState.status === 'unverified' || cviState.status === 'onboarding'}>
                {isMinting ? (
                  <>
