...
export function TerminalNav() {
  ...
  return (
    <div className="sticky top-0 z-50 bg-[#0a0a0a] border-b border-[#1f1f1f]">
      <nav className="h-12 px-6 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          {mounted && <div className="network-badge hidden sm:block">{chainLabel}</div>}

          {/* Mobile hamburger */}
          <button
            className="md:hidden p-1.5 text-[#666666] hover:text-[#e5e5e5] transition-colors"
      ...
      {/* Mobile dropdown */}
      {mobileMenuOpen && (
        <div className="md:hidden border-t border-[#1f1f1f] px-4 py-2 space-y-1 bg-[#0a0a0a]">
          {navItems.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className={cn(
                "block px-3 py-2 text-xs transition-colors rounded",
                isActive(item.href)
                  ? "text-[#e5e5e5] bg-[#1a1a1a]"
                  : "text-[#666666] hover:text-[#e5e5e5]"
              )}
          ...
          {mounted && isConnected && address ? (
            <button
              onClick={() => disconnect()}
              className="w-full text-left px-3 py-2 text-xs text-[#666666] hover:text-[#e5e5e5] transition-colors font-mono"
            >
            ...
            <button
              onClick={() => connect({ connector: injected() })}
              disabled={!mounted || isPending}
              className="w-full text-left px-3 py-2 text-xs text-[#666666] hover:text-[#e5e5e5] transition-colors disabled:opacity-50"
            >
