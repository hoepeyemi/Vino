...
import { createMonadTestnetTransport, getMonadTestnetRpcUrls } from "../monad-rpc"
...
const MONAD_TESTNET_CHAIN = {
  ...
  rpcUrls: {
    default: { http: getMonadTestnetRpcUrls() },
    public: { http: getMonadTestnetRpcUrls() },
  },
  blockExplorers: {
...
// Get chain based on environment (defaults to Monad Testnet for testnet)
const chainId = Number(process.env.NEXT_PUBLIC_CHAIN_ID || CHAIN_IDS.MONAD_TESTNET)
...
export const publicClient = createPublicClient({
  chain,
  transport: chain.id === CHAIN_IDS.MONAD_TESTNET
